...
class AppController:
    ...
    def __init__(self, page: ft.Page):
        ...
        # === ESTADOS DO APP ===
        self.current_view = "auth"  # Define a tela inicial como a tela de autenticação
        ...
        # === INICIALIZAR CONTROLLERS ADICIONAIS ===
        self.auth_controller = None  # Será inicializado quando a view de auth for carregada
    ...
    def get_current_view(self) -> ft.Control:
        ...
        elif self.current_view == "auth":
            # Carrega a view de autenticação
            from views.auth_view import AuthView
            # Inicializa o controller de autenticação se necessário
            if self.auth_controller is None:
                from controllers.auth_controller import AuthController
                self.auth_controller = AuthController(self.page, self)
            return AuthView(self.auth_controller).build()
        elif self.current_view == "academic_form":
            # Carrega a view do formulário acadêmico
            from views.academic_form_view import AcademicFormView
            # Garante que o controller de autenticação está inicializado
            if self.auth_controller is None:
                from controllers.auth_controller import AuthController
                self.auth_controller = AuthController(self.page, self)
            return AcademicFormView(self.auth_controller).build()
        else:
    ...
    # === MÉTODOS DE NAVEGAÇÃO PARA AS NOVAS VIEWS ===
    
    def navigate_to_academic_form(self):
        """
        Navega para a tela de formulário acadêmico
        """
        print("➡️ Navegando para o formulário acadêmico")
        self.current_view = "academic_form"
        self.update_view()
    
    def navigate_to_roadmap(self):
        """
        Navega para a tela de roadmap (mapa de fases)
        """
        print("➡️ Navegando para o roadmap")
        self.current_view = "roadmap"
        self.update_view()
